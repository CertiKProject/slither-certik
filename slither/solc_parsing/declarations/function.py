import logging
from typing import Dict, Optional, Union, List, TYPE_CHECKING, Tuple, Literal, Set
...
class FunctionSolc(CallerContextExpression):
    ...
    def _parse_try_catch(self, statement: Dict, node: NodeSolc, scope: Scope) -> NodeSolc:
        ...
        if len(try_params) > 0:
            ...
            for v in try_params:
                ...
                new_node = self._parse_variable_definition(new_statement, new_node, catch_scope)
            tuple_expression = {
                "nodeType" : "TupleExpression",
                "src": statement["src"],
        ...
        self._parse_catch(try_clause, try_catch_node, True, catch_scope)

        for clause in catch_clauses:
            self._parse_catch(clause, try_catch_node, False, catch_scope)
    ...
    def _parse_catch(self, statement: Dict, node: NodeSolc, is_try_clause : bool, scope: Scope) -> NodeSolc:
        ...
        if (not is_try_clause) and params and (params[0]["name"] != ""):
            ...
            new_node = self._parse_variable_definition(new_statement, try_node, try_scope)
        else:
            new_node = try_node
    ...
    def _parse_cfg(self, cfg: Dict) -> None:
        ...
        if not statements:
            ...
            node = self._parse_block(cfg, node, self.underlying_function)
